...
# 載入數據集
X_no_sc_original = np.genfromtxt("C:/Users/Jane/Desktop/NTUST-A/中正/autoencoder/DDOS_Autoencoder.csv", delimiter=',', dtype="float64")
...
sm = SMOTE(ratio={0: 62000,1: 62000}, random_state=1)
X_no_sc_original, Y_normal_original = sm.fit_sample(X_no_sc_original, Y_normal_original)
print(Counter(Y_normal_original))

# 載入數據集
#X_no_sc_original = np.genfromtxt("C:/Users/Jane/Desktop/NTUST-A/中正/autoencoder/csv_encoder_result1.csv", delimiter=',', dtype="float64")
...
# 輸出層
W4 = tf.Variable(tf.truncated_normal([Hidden_2, Output_node], stddev=Hidden_stddev))
...
regularizers = tf.nn.l2_loss(W1)+tf.nn.l2_loss(W2)+tf.nn.l2_loss(W4)
...
with tf.Session(config=tf.ConfigProto()) as sess:
    ...
    while True:
        ...
        if (stopping_step >= Stop_Threshold):
            ...
            f = open('G05stddv03lrate_back_adam.txt', 'a')
            lines = ['epoch:' + str(epoch), '\n', 'Testing Accuracy:' + str(test_acc), '\n',
                     'Training Accuracy:' + str(train_acc), '\n']
            f.writelines(lines)
            f.close()
            break;
        elif (epoch >= 1e-6 and abs(fig_test_loss[epoch - 1] - fig_test_loss[epoch]) < 1e-6):
            epoch = epoch
            print('early stopping', epoch)
            f = open('G06stddv02lrate_back_adam.txt', 'a')
            lines = ['epoch:' + str(epoch), '\n', 'Testing Accuracy:' + str(test_acc), '\n',
                     'Training Accuracy:' + str(train_acc), '\n']
            f.writelines(lines)
            f.close()
    ...
    plt.savefig('G06stddv03lrate_acc.png', dpi=800)
    plt.show()
    ...
    plt.savefig('G06stddv03lrate_loss.png', dpi=800)
    plt.show()
    ...
    plt.savefig('G06stddv03lrate_confusionmatrix.png', dpi=800)
    plt.show()
