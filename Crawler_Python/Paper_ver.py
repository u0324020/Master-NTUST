...
def Add_CSV(arr):
    ...
    df.to_csv("C:/Users/Jane/Desktop/NTU/Scam/Data/Malicious/Malicious_RawData_paper.csv", encoding="utf-8",
...
def Network_based(ID, df):
    ...
    url = str(df.url[ID])
    URL_keywords = ["techsupport", "alert", "pc", "security", "windows"]
...
def URL_whois(ID, df):
    ...
    First_list[16] = round((de_creation / 100000), 1)
    First_list[17] = round((de_update / 100000), 1)
    First_list[18] = round((de_expiration / 100000), 1)
    First_list[19] = round((de_once_update / 100000), 1)


def Open_txt(now_ID):
    txt = open("C:/Users/Jane/Desktop/NTU/Scam/Data/Malicious/M_txt/" +
               str(now_ID) + ".txt", "r", encoding="utf-8")
    content = (txt.read()).lower()
    # print(content)
    count_keywords = 0
    brands = ["microsoft", "apple", "dell",
              "yahoo", "hp", "mcafee", "facebook"]
    for i in brands:
    ...
    keywords = ["support", "phone", "contact", "free", "protect", "update", "window", "alert", "document", "eval", "var",
                ...
                "window.location", "window.AudioContext", "window.onbeforeunload", "computer", "repair", "call centre"]
    location = 22
    for i in keywords:
        if i.lower() in content:
            print(i)
...
if __name__ == '__main__':
    file_path = "C:/Users/Jane/Desktop/NTU/Scam/Data/Malicious/Malicious_Org_0-33030.csv"
    First_list = [0] * 48
